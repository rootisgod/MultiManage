...
def UpdateInstanceTableValues():
    """
    Update the instance table values by querying multipass for current instances.
    Updates the global instancesHeadersForTable and instancesDataForTable variables.
...
while True:
    ...
    if event == '-CREATEINSTANCE-':
        ...
        command_args = ['multipass', 'launch', itype, '-v', '-n', iname, '-c', icpus, '-m', iram, '-d', idisk]
        if values["-USECLOUDINIT-"] == True:
            f = open(f"{local_cloud_init_yaml_filename}", "w")
            f.write(values["-CLOUDINITYAML-"])
            f.close()
            command_args.append(f'--cloud-init {working_folder}/{local_cloud_init_yaml_filename}')
        if values['-INPMOUNTFOLDER-'] != '':
            ...
            if os.path.isdir(mountFolderValue) == False:
                ...
                command_args.append(f'--mount {mountFolderValue}:/multipass')
        if stop_creation != True:
            ...
            if platform.system() in ("Windows"):
                retval = runCommandInTerminalWindow(command_args)
                ...
                runCommand(cmd=command_args, window=window)
    ...
    if event == '-STARTBUTTON-':
        ...
        command_args = ['multipass', 'start', selectedInstanceName, '-v']
        runCommand(cmd=command_args, window=window)
    ...
    if event == '-RESTARTBUTTON-':
        ...
        command_args = ['multipass', 'restart', selectedInstanceName, '-v']
        runCommand(cmd=command_args, window=window)
    ...
    if event == '-STOPBUTTON-':
        ...
        command_args = ['multipass', 'stop', selectedInstanceName, '-v']
        runCommand(cmd=command_args, window=window)
    ...
    if event == '-DELETEBUTTON-':
        ...
        command_args = ['multipass', 'delete', selectedInstanceName, '-v']
        runCommand(cmd=command_args, window=window)
        ...
        if purgeAll == 'Yes':
            command_args = ['multipass', 'purge', '-v']
            runCommand(cmd=command_args, window=window)
